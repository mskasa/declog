...
- [x] GitHub PR auto-comment — when a PR modifies files listed in any document's Related Files section, CI automatically comments with the related document links; the existing `adr-check.yml` only checks whether an ADR was committed, not whether existing ADRs are relevant to the PR
