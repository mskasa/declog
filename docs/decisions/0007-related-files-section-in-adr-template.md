...
The planned `why blame <file>` command needs to find ADRs related to a given file path.
...
A structured section in the template gives authors a clear place to record which files a decision affects, and gives `why blame` a reliable target to search.
...
- `why blame <file>` can search the Related Files section for reliable, low-noise results
...
- **Full-text search without a dedicated section:** Simpler template, but `why blame` would match incidental mentions (e.g. a file name appearing in an example) alongside intentional associations
...
- **Optional section:** Reduces noise in simple ADRs but makes `why blame` unreliable when the section is absent
