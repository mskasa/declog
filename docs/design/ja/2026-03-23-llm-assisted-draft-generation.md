...
- **変更ファイル**: `git diff --staged --name-only` によるステージ済みファイル
- **ステージ済み diff**: `git diff --staged` の出力。トークン予算を考慮して `DiffLimit = 2000` 文字で切り捨て

どちらもステージ済みファイルのみを対象とする。`kizami adr --ai` を実行する前に `git add` することで、開発者が送信するコンテキストを明示的にコントロールできる。diff の切り捨ては、実装をシンプルかつ決定論的に保つために行う。
...
- **未ステージの変更**: 変更ファイル一覧と diff はどちらもステージ済みファイルのみに限定している。これは意図的な設計で、`kizami adr --ai` 実行前にステージすることで開発者がコンテキストをコントロールする。未ステージの変更は無関係な作業中の編集を含むリスクがあるため除外する。
