...
| フラグ | 説明 |
|---|---|
| `--dry-run` | APIを呼ぶ前に送信されるプロンプトを表示する。APIコストが発生する前に入力内容を確認したい場合に便利 |
| `--model <model-id>` | このコマンド実行時のAnthropicモデルを上書きする（デフォルト：`claude-sonnet-4-20250514`、または `kizami.toml` の設定値） |
