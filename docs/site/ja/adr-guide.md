...
| フラグ | 説明 |
|---|---|
| `--dry-run` | AIバックエンドを呼ぶ前に送信されるプロンプトを表示する。APIコストが発生する前に入力内容を確認したい場合に便利 |
| `--model <model-id>` | このコマンド実行時のモデルを上書きする（デフォルト：`claude-sonnet-4-20250514`、または `kizami.toml` の設定値）。BedrockのモデルIDを渡すと自動的にBedrockバックエンドが有効になる |
...
デフォルトのトリガーは main ブランチへの push です。チームのブランチ戦略に合わせて、別のブランチや別のタイミング（PRマージ時など）に変更してください。
...
重要なファイルはMarkdownだけではありません。SQLスキーマ・OpenAPI仕様書・CSVテストマトリクス——これらもドキュメントと同様に静かに乖離していきますが、kizamiのマーカーを直接埋め込むことはできません。

そのようなファイルには `.kizami` サイドカーファイルを隣に置きます：
